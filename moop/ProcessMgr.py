...
from moop.utilities import (
    compute_cosine_distance,
    get_device,
    str_to_class,
    shuffle_array,
)
import moop.vr_util as vr

from typing import Any, List, Callable
from moop.typing import Frame, Face
from concurrent.futures import ThreadPoolExecutor, as_completed
from threading import Thread, Lock
from queue import Queue
from tqdm import tqdm
from moop.ffmpeg_writer import FFMPEG_VideoWriter
from moop.StreamWriter import StreamWriter
import moop.globals
...
class ProcessMgr:
    ...
    def initialize(self, input_faces, target_faces, options):
        ...
        moop.globals.g_desired_face_analysis = [
            "landmark_3d_68",
            "landmark_2d_106",
            "detection",
            "recognition",
        ...
        if options.swap_mode == "all_female" or options.swap_mode == "all_male":
            moop.globals.g_desired_face_analysis.append("genderage")
        ...
        for f in current_files:
            ...
            if temp_frame is not None:
                ...
                if resimg is not None:
                    ...
                    cv2.imencode(f".{moop.globals.CFG.output_image_format}", resimg)[
                        1
    ...
    def read_frames_thread(self, cap, frame_start, frame_end, num_threads):
        ...
        while True and moop.globals.processing:
            ret, frame = cap.read()
            if not ret:
                break
    ...
    def run_batch_inmem(
        ...
        if self.output_to_file:
            self.videowriter = FFMPEG_VideoWriter(
                ...
                (width, height),
                fps,
                codec=moop.globals.video_encoder,
                crf=moop.globals.video_quality,
                audiofile=None,
            )
        if self.output_to_cam:
    ...
    def process_frame(self, frame: Frame):
        ...
        if num_swapped > 0:
            if moop.globals.no_face_action == eNoFaceAction.SKIP_FRAME_IF_DISSIMILAR:
                if len(self.input_face_datas) > num_swapped:
                    return None
            self.num_frames_no_face = 0
            self.last_swapped_frame = temp_frame.copy()
            return temp_frame
        if moop.globals.no_face_action == eNoFaceAction.USE_LAST_SWAPPED:
            if (
        ...
        elif moop.globals.no_face_action == eNoFaceAction.USE_ORIGINAL_FRAME:
            return frame
        if moop.globals.no_face_action == eNoFaceAction.SKIP_FRAME:
    ...
    def swap_faces(self, frame, temp_frame):
        ...
        if moop.globals.vr_mode and num_faces_found % 2 > 0:
    ...
    def process_face(self, face_index, target_face: Face, frame: Frame):
        from moop.face_util import align_crop
        ...
        if moop.globals.autorotate_faces:
