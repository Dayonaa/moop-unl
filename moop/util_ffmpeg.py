...
def extract_frames(
    ...
    commands.extend(
        [
            ...
            "0",
            os.path.join(
                temp_directory_path, "%06d." + moop.globals.CFG.output_image_format
            ),
        ]
    )
...
def create_video(
    ...
    run_ffmpeg(
        [
            ...
            "-i",
            os.path.join(
                temp_directory_path, f"%06d.{moop.globals.CFG.output_image_format}"
            ),
            "-c:v",
            moop.globals.video_encoder,
            "-crf",
            str(moop.globals.video_quality),
            "-pix_fmt",
            "yuv420p",
            "-vf",
...
def create_gif_from_video(video_path: str, gif_path):
    from moop.capturer import get_video_frame, release_video
