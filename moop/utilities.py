...
def sort_rename_frames(path: str):
    ...
    for i in range(len(filenames)):
        ...
        new_filename = os.path.join(
            path, f"{newidx:06d}." + moop.globals.CFG.output_image_format
        )
...
def replace_template(file_path: str, index: int = 0) -> str:
    ...
    template = moop.globals.CFG.output_template
    replaced_filename = template_parser.parse(
        template, {"index": str(index), "file": fn}
    )

    return os.path.join(moop.globals.output_path, f"{replaced_filename}{ext}")
...
def clean_temp(target_path: str) -> None:
    ...
    if not moop.globals.keep_frames and os.path.isdir(temp_directory_path):
...
def get_device() -> str:
    if len(moop.globals.execution_providers) < 1:
        moop.globals.execution_providers = ["CPUExecutionProvider"]

    prov = moop.globals.execution_providers[0]
...
def conditional_thread_semaphore() -> Union[Any, Any]:
    if (
        "DmlExecutionProvider" in moop.globals.execution_providers
        or "ROCMExecutionProvider" in moop.globals.execution_providers
    ):
