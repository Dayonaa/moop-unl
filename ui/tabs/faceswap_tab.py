...
import moop.globals
import ui.globals
from moop.face_util import extract_face_images, create_blank_image
from moop.capturer import get_video_frame, get_video_frame_total, get_image_frame
from moop.ProcessEntry import ProcessEntry
from moop.ProcessOptions import ProcessOptions
from moop.FaceSet import FaceSet
from moop.utilities import clean_dir
...
def faceswap_tab():
        ...
        with gr.Row(variant="panel"):
            ...
            with gr.Column(scale=1):
                with gr.Group():
                    ...
                    moop.globals.skip_audio = gr.Checkbox(
                        label="Skip audio", value=False
                    )
                    moop.globals.keep_frames = gr.Checkbox(
                        label="Keep Frames (relevant only when extracting frames)",
                        value=False,
                    )
                    moop.globals.wait_after_extraction = gr.Checkbox(
        ...
        with gr.Row(variant="panel"):
            ...
            with gr.Column():
                ...
                gr.Button("👀 Open Output Folder", size="sm").click(
                    fn=lambda: util.open_folder(moop.globals.output_path)
                )
            with gr.Column(scale=2):
    ...
    start_event = bt_start.click(
        ...
        inputs=[
            ...
            ui.globals.ui_selected_enhancer,
            selected_face_detection,
            moop.globals.keep_frames,
            moop.globals.wait_after_extraction,
            moop.globals.skip_audio,
            max_face_distance,
            ui.globals.ui_blend_ratio,
            selected_mask_engine,
            clip_text,
...
def set_mask_offset(index, mask_offset):
    ...
    if len(moop.globals.INPUT_FACESETS) > SELECTED_INPUT_FACE_INDEX:
        offs = (
            moop.globals.INPUT_FACESETS[SELECTED_INPUT_FACE_INDEX].faces[0].mask_offsets
        )
        ...
        if offs[2] + offs[3] > 0.99:
            offs[2] = 0.99
            offs[3] = 0.0
        moop.globals.INPUT_FACESETS[SELECTED_INPUT_FACE_INDEX].faces[
            0
        ].mask_offsets = offs
...
def remove_selected_input_face():
    ...
    if len(moop.globals.INPUT_FACESETS) > SELECTED_INPUT_FACE_INDEX:
        f = moop.globals.INPUT_FACESETS.pop(SELECTED_INPUT_FACE_INDEX)
        del f
...
def move_selected_input(button_text):
    ...
    f = moop.globals.INPUT_FACESETS.pop(SELECTED_INPUT_FACE_INDEX)
    moop.globals.INPUT_FACESETS.insert(SELECTED_INPUT_FACE_INDEX + offset, f)
    f = ui.globals.ui_input_thumbs.pop(SELECTED_INPUT_FACE_INDEX)
...
def move_selected_target(button_text):
    ...
    f = moop.globals.TARGET_FACES.pop(SELECTED_TARGET_FACE_INDEX)
    moop.globals.TARGET_FACES.insert(SELECTED_TARGET_FACE_INDEX + offset, f)
...
def on_use_face_from_selected(files, frame_num):
    ...
    moop.globals.target_path = files[selected_preview_index].name
    if util.is_image(
        moop.globals.target_path
    ) and not moop.globals.target_path.lower().endswith(("gif")):
        SELECTION_FACES_DATA = extract_face_images(moop.globals.target_path, (False, 0))
        if len(SELECTION_FACES_DATA) > 0:
            ...
            moop.globals.target_path = None

    elif util.is_video(
        moop.globals.target_path
    ) or moop.globals.target_path.lower().endswith(("gif")):
        selected_frame = frame_num
        SELECTION_FACES_DATA = extract_face_images(
            moop.globals.target_path, (True, selected_frame)
        )
        if len(SELECTION_FACES_DATA) > 0:
            for f in SELECTION_FACES_DATA:
                image = util.convert_to_gradio(f[1])
    ...
    if len(thumbs) == 1:
        moop.globals.TARGET_FACES.append(SELECTION_FACES_DATA[0][0])
...
def on_selected_face():
    ...
    if IS_INPUT:
        ...
        moop.globals.TARGET_FACES.append(fd[0])
...
def on_preview_frame_changed(
    ...
    from moop.core import live_swap, get_processing_plugins

    manual_masking = False
    mask_offsets = (0, 0, 0, 0)
    if len(moop.globals.INPUT_FACESETS) > SELECTED_INPUT_FACE_INDEX:
        if not hasattr(
            moop.globals.INPUT_FACESETS[SELECTED_INPUT_FACE_INDEX].faces[0],
            "mask_offsets",
        ):
            moop.globals.INPUT_FACESETS[SELECTED_INPUT_FACE_INDEX].faces[
                0
    ...
    if not fake_preview or len(moop.globals.INPUT_FACESETS) < 1:
        return (
    ...
    moop.globals.face_swap_mode = translate_swap_mode(detection)
    moop.globals.selected_enhancer = enhancer
    moop.globals.distance_threshold = face_distance
    moop.globals.blend_ratio = blend_ratio
    moop.globals.no_face_action = index_of_no_face_action(no_face_action)
    moop.globals.vr_mode = vr_mode
    moop.globals.autorotate_faces = auto_rotate
    moop.globals.subsample_size = int(upsample[:3])
    ...
    if len(moop.globals.INPUT_FACESETS) <= face_index:
        face_index = 0
...
def on_clear_input_faces():
    ...
    moop.globals.INPUT_FACESETS.clear()
    return ui.globals.ui_input_thumbs


def on_clear_destfiles():
    moop.globals.TARGET_FACES.clear()
...
def start_swap(
    ...
    from moop.core import batch_process_regular
    ...
    if moop.globals.CFG.clear_output:
        clean_dir(moop.globals.output_path)
    ...
    moop.globals.distance_threshold = face_distance
    moop.globals.blend_ratio = blend_ratio
    moop.globals.keep_frames = keep_frames
    moop.globals.wait_after_extraction = wait_after_extraction
    moop.globals.skip_audio = skip_audio
    moop.globals.face_swap_mode = translate_swap_mode(detection)
    moop.globals.no_face_action = index_of_no_face_action(no_face_action)
    moop.globals.vr_mode = vr_mode
    moop.globals.autorotate_faces = autorotate
    moop.globals.subsample_size = int(upsample[:3])
    mask_engine = map_mask_engine(selected_mask_engine, clip_text)

    if moop.globals.face_swap_mode == "selected":
        if len(moop.globals.TARGET_FACES) < 1:
    ...
    is_processing = False
    outdir = pathlib.Path(moop.globals.output_path)
...
def get_gradio_output_format():
    if moop.globals.CFG.output_image_format == "jpg":
        return "jpeg"
    return moop.globals.CFG.output_image_format


def display_output(filename):
    if util.is_video(filename) and moop.globals.CFG.output_show_video:
