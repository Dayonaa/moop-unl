...
def settings_tab():
    from moop.core import suggest_execution_providers
        ...
        with gr.Row():
            with gr.Column():
                themes = gr.Dropdown(
                    ...
                    value=moop.globals.CFG.selected_theme,
                )
            with gr.Column():
                settings_controls.append(
                    gr.Checkbox(
                        label="Public Server",
                        value=moop.globals.CFG.server_share,
                ...
                settings_controls.append(
                    gr.Checkbox(
                        ...
                        value=moop.globals.CFG.clear_output,
                ...
                output_template = gr.Textbox(
                    ...
                    value=moop.globals.CFG.output_template,
                )
            with gr.Column():
                input_server_name = gr.Textbox(
                    ...
                    value=moop.globals.CFG.server_name,
                )
            with gr.Column():
                input_server_port = gr.Number(
                    ...
                    value=moop.globals.CFG.server_port,
                )
        with gr.Row():
            with gr.Column():
                settings_controls.append(
                    gr.Dropdown(
                        providerlist,
                        label="Provider",
                        value=moop.globals.CFG.provider,
                        elem_id="provider",
                        interactive=True,
                    )
                )
                ...
                settings_controls.append(
                    gr.Checkbox(
                        ...
                        value=moop.globals.CFG.force_cpu,
                        elem_id="force_cpu",
                        interactive=True,
                    )
                )
                max_threads = gr.Slider(
                    1,
                    32,
                    value=moop.globals.CFG.max_threads,
            ...
            with gr.Column():
                memory_limit = gr.Slider(
                    0,
                    128,
                    value=moop.globals.CFG.memory_limit,
                ...
                settings_controls.append(
                    gr.Dropdown(
                        ...
                        info="default: png",
                        value=moop.globals.CFG.output_image_format,
            ...
            with gr.Column():
                settings_controls.append(
                    gr.Dropdown(
                        ...
                        value=moop.globals.CFG.output_video_codec,
                ...
                settings_controls.append(
                    gr.Dropdown(
                        ...
                        info="default: mp4",
                        value=moop.globals.CFG.output_video_format,
                ...
                video_quality = gr.Slider(
                    0,
                    100,
                    value=moop.globals.CFG.video_quality,
                ...
                with gr.Group():
                    settings_controls.append(
                        gr.Checkbox(
                            ...
                            value=moop.globals.CFG.use_os_temp_folder,
                    ...
                    settings_controls.append(
                        gr.Checkbox(
                            ...
                            value=moop.globals.CFG.output_show_video,
...
def on_option_changed(evt: gr.SelectData):
    ...
    if isinstance(evt.target, gr.Checkbox):
        if hasattr(moop.globals, attribname):
            setattr(moop.globals, attribname, evt.selected)
            return
    elif isinstance(evt.target, gr.Dropdown):
        if hasattr(moop.globals, attribname):
            setattr(moop.globals, attribname, evt.value)
            return
    raise gr.Error(f"Unhandled Setting for {evt.target}")


def on_settings_changed_misc(new_val, attribname):
    if hasattr(moop.globals.CFG, attribname):
        setattr(moop.globals.CFG, attribname, new_val)
    else:
...
def on_settings_changed(evt: gr.SelectData):
    ...
    if isinstance(evt.target, gr.Checkbox):
        if hasattr(moop.globals.CFG, attribname):
            setattr(moop.globals.CFG, attribname, evt.selected)
            return
    elif isinstance(evt.target, gr.Dropdown):
        if hasattr(moop.globals.CFG, attribname):
            setattr(moop.globals.CFG, attribname, evt.value)
            return
...
def apply_settings(themes, input_server_name, input_server_port, output_template):
    ...
    moop.globals.CFG.selected_theme = themes
    moop.globals.CFG.server_name = input_server_name
    moop.globals.CFG.server_port = input_server_port
    moop.globals.CFG.output_template = output_template
    moop.globals.CFG.save()
