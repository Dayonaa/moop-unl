...
from moop.capturer import get_video_frame, get_video_frame_total
from typing import List, Tuple, Optional
from moop.typing import Frame, Face, FaceSet
...
def on_fb_files_changed(
    ...
    for f in inputfiles:
        ...
        if util.has_image_extension(source_path):
            ...
            SELECTION_FACES_DATA = extract_face_images(
                moop.globals.source_path, (False, 0), 0.5
            )
            for f in SELECTION_FACES_DATA:
                image = f[1]
                images.append(image)
...
def on_update_clicked() -> Optional[str]:
    ...
    for index, img in enumerate(images):
        filename = os.path.join(moop.globals.output_path, f"{index}.png")
        cv2.imwrite(filename, img)
        imgnames.append(filename)

    finalzip = os.path.join(moop.globals.output_path, "faceset.fsz")
